...
def find_host_uid_if_exist(api_client, cloned_host_name, cloned_host_ip):
    ...
    if res.success is False:
        if "code" in res.data and "generic_err_object_not_found" == res.data["code"]:
            ...
            discard_write_to_log_file(api_client,"Operation failed:\n{}\nAborting all changes.".format(res.error_message))
            return False
...
def main(argv):
    ...
    if argv:
        ...
        global_domain_input = input("Enter global domain name in case of MDS server: ""[In order to use the default value ('Global') or in case of CMA hit 'Enter']")
